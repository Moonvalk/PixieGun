...
namespace Moonvalk.Utilities
{
    ...
    public class MoonActionMap<EnumType> : BaseMoonActionMap<EnumType, Action>
    {
        ...
        /// <param name="clearAfterRun_">
        /// Flag that when true will clear all actions stored for this event
        /// after execution.
        /// </param>
        public MoonActionMap<EnumType> Run(EnumType event_, bool clearAfterRun_ = false)
        {
            foreach (var eventItem in Events[event_].Value)
                eventItem?.Invoke();

            if (clearAfterRun_) Clear(event_);
    ...
    public class MoonActionMap<EnumType, ParamType> : BaseMoonActionMap<EnumType, Action<ParamType>>
    {
        ...
        /// after execution.
        /// </param>
        public MoonActionMap<EnumType, ParamType> Run(EnumType event_, ParamType param_ = default, bool clearAfterRun_ = false)
        {
            foreach (var eventItem in Events[event_].Value)
                eventItem?.Invoke(param_);
    ...
    public class MoonActionMap<EnumType, ParamType, ParamType2> : BaseMoonActionMap<EnumType, Action<ParamType, ParamType2>>
    {
        ...
        /// after execution.
        /// </param>
        public MoonActionMap<EnumType, ParamType, ParamType2> Run(EnumType event_, ParamType param_ = default, ParamType2 param2_ = default,
            bool clearAfterRun_ = false)
        {
            foreach (var eventItem in Events[event_].Value)
                eventItem?.Invoke(param_, param2_);
