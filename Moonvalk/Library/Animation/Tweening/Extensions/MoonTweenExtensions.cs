...
namespace Moonvalk.Animation
{
    ...
    public static class MoonTweenExtensions
    {
        ...
        public static Action GetRemoveAction(Node object_, MoonTweenProperty property_)
        {
            return () =>
            {
                ...
                if (TweenGroups.ContainsKey(object_))
                {
                    ...
                    if (group.TweenHandlers.Remove(property_) && group.TweenHandlers.Count == 0) TweenGroups.Remove(object_);
                }
            };
        }
        ...
        public static void ClearAll()
        {
            if (TweenGroups != null)
            {
                foreach (var group in TweenGroups.Values) group?.Clear();
