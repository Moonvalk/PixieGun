...
namespace Moonvalk.Components {
	...
	public abstract class MoonScenePathLoader<SceneType> : BaseSceneLoader<MoonStringArray, SceneType> where SceneType : Node {
		...
		protected override void swapToScene(int sceneIndex_) {
			MoonResourceLoader.Load<PackedScene>(this.getScenePath(sceneIndex_), (PackedScene scene_) => {
				this.RemoveChildren(this.CurrentScene);
				this.instantiateScene(scene_);
		...
		protected virtual void instantiateScene(PackedScene packedScene_) {
			this.CurrentScene = this.AddInstance<SceneType>(packedScene_);
		}
		...
		protected string getScenePath(int sceneIndex_) {
			int index = Mathf.Clamp(sceneIndex_, 0, this.Scenes.Length - 1);
			return this.Scenes.Items[index].AsPath;
		}
		#endregion
	}
}
